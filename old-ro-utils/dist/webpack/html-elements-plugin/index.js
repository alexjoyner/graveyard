...
function createTag(tagName, attrMap, publicPath) {
	...
	var attributes = Object.getOwnPropertyNames(attrMap).filter(function (name) {
		return name[0] !== '=';
	}).map(function (name) {
		...
		if (publicPath) {
			...
			var usePublicPath = attrMap.hasOwnProperty('=' + name) ? !!attrMap['=' + name] : name === 'href';
...
function getHtmlElementString(dataSource, publicPath) {
	return Object.getOwnPropertyNames(dataSource).map(function (name) {

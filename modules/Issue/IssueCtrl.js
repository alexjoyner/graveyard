'use strict';
angular.module('angular.controller.IssueCtrl', []).controller('IssueCtrl', ['$scope', '$state', 'issuesService',
    function($scope, $state, issuesService) {
        // Get issues data
        issuesService.getIssue($state.params.id).then(function(res) {
        ...
        $scope.newDataApi = {
            addMainProPoint: function() {
                ...
                issuesService.addPro($scope.issueData._id, $scope.issueTemplate).then(function() {
                    init();
                });
            },
            addMainConPoint: function() {
                $scope.issueTemplate.subSupport[0].sources = $scope.issueTemplate.subSupport[0].sources.split(',');
                issuesService.addCon($scope.issueData._id, $scope.issueTemplate).then(function(res) {
                    init();
                });
            },
        ...
        $scope.deleteMainPoint = function(id, type) {
            issuesService.deleteMainPoint($scope.issueData._id, id, type).then(function(res) {});
        };
    }
]);